...
?>
<div class="product-price" id="productPrice<?php echo $product->virtuemart_product_id ?>" {VM_PRICE_WRAPPER}>
	<?php
	...
	//if ($showBasePrice) {
		...
		$priceDiv = $currency->createPriceDiv ('basePrice', 'COM_VIRTUEMART_PRODUCT_BASEPRICE', $product->prices);
		echo str_replace('class="PricebasePrice"', $token.' class="PricebasePrice"', $priceDiv);
	...
	echo $currency->createPriceDiv ('unitPrice', $unitPriceDescription, $product->prices);
		...
		if($product->product_in_stock)	echo '{VM_PRODUCT_IN_STOCK}';
