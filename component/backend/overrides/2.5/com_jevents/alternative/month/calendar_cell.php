...
class EventCalendarCell_alternative extends EventCalendarCell_default
{
	...
	function calendarCell(&$currentDay, $year, $month, $i)
	{
		...
		#       J4Schema       #
		########################
		$cellStart	= '<div {JE_EVENT_WRAPPER} class="eventfull"><div class="eventstyle" ' ;
		$cellStyle  = 'padding:0;';
		$cellEnd	= '</div></div>' . "\n";
		...
		if ($currentDay['countDisplay'] < $cfg->get('com_calMaxDisplay', 5))
		{
			...
			else
			{
				...
				if ($title_event_link=="")
				{
					$j4sTime = date('Y-m-d H:i:s', $this->event->_unixstarttime);
					$titleTime  = '<time {JE_EVENT_STARTTIME:'.$j4sTime.'}>';
					$titleTime .= ($cfg->get('com_calDisplayStarttime') ? $tmp_start_time : '');
					$titleTime .= '</time>';

					$j4sTitle	= '<span {JE_EVENT_NAME}>';
					$j4sTitle  .= $tmpTitle;
					$j4sTitle  .= '</span>';

					$title_event_link = "\n".'<a class="cal_titlelink" href="' . $link . '" '.$linkStyle.'>'
					.$titleTime.' '.$j4sTitle.'</a>'."\n";

					$endTime  = date('Y-m-d H:i:s', $this->event->_unixendtime);
					$duration = $this->event->_unixendtime - $this->event->_unixstarttime;

					$title_event_link .= '{JE_EVENT_ENDTIME:'.$endTime.'} ';
					$title_event_link .= '{JE_EVENT_DURATION:P'.$duration.'}';
				}
