	...
	initialize : function(options)
	{
		...
		this.jsonAttrib = new Request.JSON({
			...
			onRequest : function(){
				...
				self.nonStdValue = '';
				//$('proprPlusTypeHolder').addClass('hidden');
				self.valuesChoose.addClass('hidden');
				self.valuesList.addClass('hidden');
				$('attrib_descr').empty().addClass('loader-bg-small');
				self.dateTime.addClass('hidden');
				$('propOnly').checked = true;
				self.schema_attr = '';
				self.warning.empty();
			},
			onError : function(){
			...
			onSuccess : function(response){
				...
				$('attrib_descr').removeClass('loader-bg-small').set('html', response.descr);
				self.valuesList.empty();
				self.calendarHolder.addClass('hidden');
				$('timeHolder').addClass('hidden');
				self.dateTime.addClass('hidden')
		...
		this.jsonType = new Request.JSON({
			...
			onRequest : function(){
				$('type_descr').empty().addClass('loader-bg-small');
				self.warning.empty();
				self.schema_type = '';
				self.schema_attr = '';
			},
			onSuccess : function(response){
				$('type_descr').removeClass('loader-bg-small').set('html', response.descr);
...
//		this.metaPropr		= $('metaProp');
		this.valuesChoose	= $('values_choose');
		this.valuesDescr	= $('values_descr');
		this.valuesList		= $('values_list');
	...
	addTypeSchema : function()
	{
		...
		if($('property').checked && holder.getFirst()) 	var html = holder.getFirst();
		else{
			if($('newDiv').checked) var element = 'div';
	...
	addAttribSchema : function()
	{
		...
		if(this.dateTime.isVisible())
		{
			...
			if(this.calendarHolder.isVisible()){
				if($('calendar').value.test(/^(\d{4})\-(\d{2})\-(\d{2})+$/)) timeISO  = $('calendar').value;
				else{
			...
			if($('timeHolder').isVisible())	timeISO += $('calendarTime').value.timeToISO(this.schema_attr);
		...
		{	//i try to add the attribute to the current tag (if any)
			...
			if(this.nonStdValue != '' && $('proprPlusType').checked)
			{
	...
	setEvents : function()
	{
		...
		$('wrap').addEvent('click', function(){$('newElement').removeClass('hidden')});
		$('property').addEvent('click', function(){$('newElement').addClass('hidden')});
		
		if($('toggleEditor')){
			$('toggleEditor').addEvent('click', function(){
				
				var editorFx = new Fx.Tween('html_code', {duration : 'short'});
	
				if($('j4sSettings').hasClass('hidden'))
				{			
					editorFx.start('height', '50px').chain(
							function(){$('j4sSettings').removeClass('hidden');}
						);
		...
		if(this.options.lite == false){
			...
			$('remove_schemas').addEvent('click', function(){self.removeSchemas()});
			...
			this.options.html_code.addEvent('mouseup', function(){
				...
				if(self.selectedText != '' && typeof self.selectedText != "undefined"){
					...
					$('currSelection').set('html', htmlentities(text));}
				else{
